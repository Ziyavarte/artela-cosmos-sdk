...
func (app *BaseApp) Commit() abci.ResponseCommit {
	...
	t := time.Now()
	fmt.Printf("_______________ms.Write, height: %d\n",
		header.Height)
	app.deliverState.ms.Write()
	fmt.Printf("_______________ms.Write, height: %d, duration: %.2fms\n",
		header.Height, float64(time.Since(t).Microseconds())/1000)
	t = time.Now()

	fmt.Printf("_______________ms.Commit, height: %d\n",
		header.Height)
	commitID := app.cms.Commit()
	fmt.Printf("_______________ms.Commit, height: %d, duration: %.2fms\n",
		header.Height, float64(time.Since(t).Microseconds())/1000)
